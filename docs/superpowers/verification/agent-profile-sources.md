...
| OpenCode | `bash {command, timeout: 600000}`, `--timeout 540` | ✓ tool id `bash`; params `command`, `timeout` (ms, positive int), `workdir`. Default 120000 (env `OPENCODE_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS`). **No hard max** in this source (spec says "hard max 10 min"); 600000 kept | `src/tool/shell/id.ts:16`; `src/tool/shell/prompt.ts:15-23`; `src/tool/shell.ts:347,540-564,615-618`; `src/effect/runtime-flags.ts:53` |
